...
public class Ship : MonoBehaviour
{
    ...
    public void CheckDone()
    {
        if (isDone)
        {
            StartCoroutine(IEDelayHandelDone());
        }
    }
    IEnumerator IEDelayHandelDone()
    {
        ...
        AudioManager.Instance.PlaySoundShipMove();
        transform.DOMoveX(23, 0.7f).OnComplete(() =>
        {
            GlobalInstance.Instance.gameManagerInstance.level.currentShip++;
